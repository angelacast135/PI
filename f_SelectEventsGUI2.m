% f_SelectEventsGUI2.m
% edited

function f_SelectEventsGUI2()
...
%     s_PeakLevelTres = 10;  
    ...
    str_FileName = 'vlade_catalina_diab';
    v_TimeSegs = [0 -1];
    s_SigInd = 8; %%%%%%%%%%%%%%%%%%
    s_PeakLevelTres = 10;
...
%     str_DataFileName = ['./CoherenceMario/' str_FileName '.mat'];
    ...
    v_EMGSigs = m_SignalsMat(s_SigInd, :)'; % y_raw
    v_Time = (0:size(v_EMGSigs, 1) - 1)./ s_SRate; %time
...
%     v_EMGFiltEnvSigs = abs(hilbert(v_EMGFiltSigs));
    ...
    [v_Peaks, v_Ind] = findpeaks(v_HistY); %, 'MinPeakProminence', 20);
    [v_Lows, v_IndLows] = findpeaks(-1.* v_HistY);
    s_Ind = v_Ind(1);
...
%     end
    ...
    s_Fig = figure;
    s_Axes_up = subplot(2,1,2);
    plot(v_SigFiltTemp./ max(v_SigFiltTemp))
    hold on
    plot(v_SigTemp, 'r')    
%     hold on, 
%     sig_9 = m_SignalsMat(9,:);
%     s_Axes_down = subplot(2,1,2);
%     plot(sig_9./ max(sig_9))
