...
%     s_Nyq = round(ps_SampleRate / 2);
    ...
    if strcmpi(ps_FilterName, 'cheby2')
        ...
        v_Wst = [];
        if isempty(ps_Order)
            if strcmpi(pstr_Type, 'low') || strcmpi(pstr_Type, 'high')
                ...
                s_HighFreq = s_LowFreq + (0.5 * 10^(-1 * s_Scale));
                v_StopFreq = s_HighFreq;
                if strcmpi(pstr_Type, 'high')
            ...
            elseif strcmpi(pstr_Type, 'stop') || isempty(pstr_Type)
                ...
                s_LowFreq = ps_CutFreqs(1) - (0.5 * 10^(-1 * s_Scale));
                s_Scale = 0;
                while s_HighFreq < 1
                    s_HighFreq = s_HighFreq * 10;
                    s_Scale = s_Scale + 1;
                end
                s_HighFreq = ps_CutFreqs(2) + (0.5 * 10^(-1 * s_Scale));
        ...
        if ps_Order > 500
            fprintf('[f_GetIIRFilter] - WARNING: Filter order of %d is too large!. Order set to 500!\n', ps_Order);
            ps_Order = 500;
        end
        if ~isempty(v_Wst)
            if ~isempty(pstr_Type)
